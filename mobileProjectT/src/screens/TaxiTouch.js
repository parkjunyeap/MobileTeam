...
export default function TaxiTouch() {
  ...
  const socket = io("http://172.30.1.76:8001");
  ...
  const toggleSwitch = async (newValue) => {
    try {
      ...
      await axios.post("http://172.30.1.76:8000/UpDriveState", newDriveState);
    } catch (error) {
  ...
  const acceptRequest = async (request) => {
    ...
    try {
      ...
      axios
        .post("http://172.30.1.76:8000/Payment", payment) // 로컬호스트/8000번으로 레지스터 Url, user 객체를줌
        .then((response) => {
          // 요청이 성공적으로 처리될 때의 로직
