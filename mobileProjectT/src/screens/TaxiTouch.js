...
export default function TaxiTouch() {
  ...
  const socket = io("http://10.20.64.189:8001");
  ...
  useEffect(
    () => {
      ...
      (async () => {
        ...
        axios
          .post("http://10.20.64.189:8000/taxiLocation", driverLocation) //
          .then((response) => {
  ...
  const toggleSwitch = async (newValue) => {
    try {
      ...
      await axios.post("http://10.20.64.189:8000/UpDriveState", newDriveState);
    } catch (error) {
  ...
  const acceptRequest = async (request) => {
    ...
    try {
      ...
      axios
        .post("http://10.20.64.189:8000/Payment", payment) // 로컬호스트/8000번으로 레지스터 Url, user 객체를줌
        .then((response) => {
          // 요청이 성공적으로 처리될 때의 로직
