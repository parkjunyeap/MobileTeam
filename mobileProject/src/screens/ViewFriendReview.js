...
const ViewFriendReview = () => {
  ...
  //         `http://10.20.34.180:8000/reviews/receiver/${selectedUserId}`
  //       );
    ...
    const fetchReviewRequest = async () => {
      try {
        ...
        if (selectedDriverId) {
          ...
          apiUrl = `http://10.20.34.180:8000/reviews/receiver/driver/${selectedDriverId}`;
        } else if (selectedUserId) {
          // 사용자에 대한 리뷰를 요청하는 경우
          apiUrl = `http://10.20.34.180:8000/reviews/receiver/user/${selectedUserId}`;
        } else {
