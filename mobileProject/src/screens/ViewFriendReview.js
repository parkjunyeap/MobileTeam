...
const ViewFriendReview = () => {
  ...
  //         `http://172.30.1.76:8000/reviews/receiver/${selectedUserId}`
  //       );
    ...
    const fetchReviewRequest = async () => {
      try {
        ...
        if (selectedDriverId) {
          ...
          apiUrl = `http://172.30.1.76:8000/reviews/receiver/driver/${selectedDriverId}`;
        } else if (selectedUserId) {
          // 사용자에 대한 리뷰를 요청하는 경우
          apiUrl = `http://172.30.1.76:8000/reviews/receiver/user/${selectedUserId}`;
        } else {
