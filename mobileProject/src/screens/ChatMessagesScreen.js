...
const ChatMessagesScreen = () => {
  ...
  const socket = io("http://172.30.1.78:8001"); // 이런식으로 socket = io(주소) 해주고.
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `http://172.30.1.78:8000/messages/${userId}/${recepientId}`
      );
  ...
  useEffect(() => {
    const fetchRecepientData = async () => {
      ...
      try {
        const response = await fetch(
          `http://172.30.1.78:8000/user/${recepientId}`
        );
  ...
  const deleteMessages = async (messageIds) => {
    try {
      const response = await fetch("http://172.30.1.78:8000/deleteMessages", {
        method: "POST",
        headers: {
