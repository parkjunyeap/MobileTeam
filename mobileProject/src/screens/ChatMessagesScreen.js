...
const ChatMessagesScreen = () => {
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `http://10.20.64.189:8000/messages/${userId}/${recepientId}`
      );
  ...
  useEffect(() => {
    const fetchRecepientData = async () => {
      ...
      try {
        const response = await fetch(
          `http://10.20.64.189:8000/user/${recepientId}`
        );
  ...
  const deleteMessages = async (messageIds) => {
    try {
      const response = await fetch("http://10.20.64.189:8000/deleteMessages", {
        method: "POST",
        headers: {
