...
const TaxiDriverList = () => {
  ...
  useEffect(() => {
    const fetchPayDrivers = async () => {
      ...
      try {
        const response = await axios.get(
          `http://172.30.1.76:8000/driverList/payment/${userId}`
        );
    ...
    const fetchDrivers = async () => {
      axios
        .get("http://172.30.1.76:8000/driverList/") // 드라이버 아이디 전부 넘김.
        .then((response) => {
    ...
    const fetchBookingDrivers = async () => {
      try {
        const response = await axios.get(
          `http://172.30.1.76:8000/driverList/booking/${userId}` // 예약한드라이버
        );
