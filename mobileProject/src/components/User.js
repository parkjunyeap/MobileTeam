...
const User = ({ item }) => {
    ...
    const fetchFriendRequests = async () => {
      try {
        ...
        const response = await fetch(
          `http://10.20.34.195:8000/friend-requests/sent/${userId}`
        );
  ...
  useEffect(() => {
    const fetchUserFriends = async () => {
      try {
        const response = await fetch(
          `http://10.20.34.195:8000/friends/${userId}`
        );
  ...
  const sendFriendRequest = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch("http://10.20.34.195:8000/friend-request", {
        // 친구추가 요청 보내기
        method: "POST",
        headers: {
